...
class MainWindow(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(input_frame, text="Сумма кредита:", width=140).grid(
            row=0, column=0, sticky="w", pady=5)
        self.CredSum_TB = ctk.CTkEntry(input_frame)
        self.CredSum_TB.grid(row=0, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(input_frame, text="Процентная ставка:", width=140).grid(
        ...
        ctk.CTkLabel(output_frame, text="Ежемесячный платёж:",
                     width=140).grid(row=0, column=0, sticky="w", pady=5)
        self.ResMonth_TB = ctk.CTkEntry(output_frame, state="disabled")
        self.ResMonth_TB.grid(row=0, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(output_frame, text="Общая сумма выплат:",
                     width=140).grid(row=1, column=0, sticky="w", pady=5)
        self.ResSum_TB = ctk.CTkEntry(output_frame, state="disabled")
        self.ResSum_TB.grid(row=1, column=1, padx=10, pady=5, sticky="ew")

        ctk.CTkLabel(output_frame, text="Переплата:", width=140).grid(
            row=2, column=0, sticky="w", pady=5)
